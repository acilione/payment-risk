...
with sync_playwright() as p:
    ...
    expect(page.locator('.demo-banner').get_by_text('Demo data', exact=True)).to_be_visible()
    ...
    expect(page.get_by_role('dialog', name='Decision details')).to_be_visible()
    expect(page.locator('.inspector .matched-rule')).to_have_count(3)
    page.screenshot(path=str(out / 'decision-inspector.png'))
    page.keyboard.press('Escape')
    expect(page.get_by_role('dialog', name='Decision details')).not_to_be_visible()
    ...
    expect(page.get_by_text('Payment processing', exact=True)).to_be_visible()
    ...
    expect(page.get_by_role('dialog', name='About Payment risk')).to_be_visible()
    page.keyboard.press('Escape')
    expect(page.get_by_role('dialog', name='About Payment risk')).not_to_be_visible()
    ...
    page.get_by_role('button', name='Toggle navigation').click()
    page.get_by_role('button', name='Risk rules', exact=False).first.click()
    expect(page.get_by_text('Default risk rules', exact=True)).to_be_visible()
    ...
    if not args.static:
        ...
        page.locator('.heading-actions').get_by_role('button', name='Use live data', exact=True).click()
        expect(page.get_by_role('alert')).to_contain_text('Live data unavailable')
